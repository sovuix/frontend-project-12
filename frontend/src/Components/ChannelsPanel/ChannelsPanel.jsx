...
const ChannelsPanel = () => {
  const notify = (text) => toast.success(text);
  // ИЗМЕНЕНИЕ: добавил функцию для ошибок
  ...
  const handleAddChannel = async (channelName) => {
    try {
      // ИЗМЕНЕНИЕ: проверка интернета
      if (!navigator.onLine) {
        notifyError("Нет интернета");
        return;
      }
      ...
      console.error(t("channel.errorCreated"), error);
      // ИЗМЕНЕНИЕ: проверка на недоступность сервера
  ...
  const handleDeleteChannel = async (channelId) => {
    try {
      ...
      setShowModal(false);
      // ИЗМЕНЕНИЕ: добавил уведомление об успехе
      notify(t("channel.channelDeleted"));
    } catch (error) {
      console.error("Ошибка удаления канала:", error);
      // ИЗМЕНЕНИЕ: заменил alert на toast
      if (error.message === "Failed to fetch") {
        notifyError("Сервер не отвечает");
      } else {
        ...
        <ModalWindow
          ...
          onSubmit={
            modalType === "add"
              ? handleAddChannel
              : modalType === "rename"
                ? // ИЗМЕНЕНИЕ: убрал notify отсюда, теперь внутри handleRenameChannel
